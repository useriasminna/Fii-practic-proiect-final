...
makeAPIRequest('GET','https://api.themoviedb.org/3/movie/'+movieid+'?api_key=71979c8dbee4788fbc86711e4e9106b4&language=en-US').then(response =>{
    console.log(data)
 ...
 movieTitle.textContent = data.title+' ('+data.release_date.substr(0,4)+')';



 const movieCover = document.createElement('img');
 movieCover.setAttribute('src','https://image.tmdb.org/t/p/w500/'+data.poster_path);
 ...
 const genrevalue=document.createElement('p');
    ...
    { 
        ...
        {
        durationvalue.textContent=Math.floor(data.runtime/60)+'h '+(data.runtime-(Math.floor(data.runtime/60)*60))+'min';
        }
        else{
...
makeAPIRequest('GET','https://api.themoviedb.org/3/movie/'+movieid+'/credits?api_key=71979c8dbee4788fbc86711e4e9106b4').then(response =>{
 console.log(data)
    ...
    // const membercastdiv=document.createElement('div');

    // for(i=0;i<9;i++)
    // {
    // const profilepic=document.createElement('img');
    // profilepic.setAttribute('src',`https://image.tmdb.org/t/p/w500${data.cast[i].profile_path}`);
    // profilepic.setAttribute('class','profilepic');

    // membercastdiv.setAttribute('class','membercastdiv'+(i+1));

    // const membername=document.createElement('p');
    // membername.setAttribute('class','membername');

    // membername.textContent=data.cast[i].name;
    // membercastdiv.appendChild(profilepic);
    // membercastdiv.appendChild(membername);
    // castdiv.appendChild(membercastdiv);
    ...
    makeAPIRequest('GET','https://api.themoviedb.org/3/movie/'+movieid+'/videos?api_key=71979c8dbee4788fbc86711e4e9106b4&language=en-US').then(response =>{
        console.log(data)
        ...
        for(i=0;i<data.results.length;i++)
        {
            ...
            {
                trailer.setAttribute('src','https://www.youtube.com/embed/'+data.results[i].key);
                break;
